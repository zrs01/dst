...
func (s *MariadbService) Load(tableFilter string) (*model.Schema, error) {
	...
	regex := regexp.MustCompile(`\w+\://[^/]*/(\w+)`).FindStringSubmatch(config.Default.Dsn)
	if len(regex) == 0 {
		return nil, tracerr.Errorf("Failed to parse %s", config.Default.Dsn)
	}

	dsName := config.Default.Dsn
	parts := strings.Split(config.Default.Dsn, "://")
	if len(parts) > 1 {
		dsName = parts[1]
	}
	...
	dbManager := NewExportBuilder().
		...
		WithCommonColumnFile(config.Default.Ccf)
