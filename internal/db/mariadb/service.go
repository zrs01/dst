...
func (s *MariadbService) Load(tableFilter string) (*model.Schema, error) {
	...
	regex := regexp.MustCompile(`\w+\://[^/]*/(\w+)`).FindStringSubmatch(config.MergeSetting.Dsn)
	if len(regex) == 0 {
		return nil, tracerr.Errorf("Failed to parse %s", config.MergeSetting.Dsn)
	}

	dsName := config.MergeSetting.Dsn
	parts := strings.Split(config.MergeSetting.Dsn, "://")
	if len(parts) > 1 {
		dsName = parts[1]
	}
	...
	dbManager := NewExportBuilder().
		...
		WithCommonColumnFile(config.MergeSetting.Ccf)
