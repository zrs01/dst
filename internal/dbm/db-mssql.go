...
func (s *MssqlService) getSchemas(db *sqlx.DB, schemaName string) (*[]MsSqlSchema, error) {
	...
	if err := db.Select(&dSchemas, "select * from INFORMATION_SCHEMA.SCHEMATA where SCHEMA_NAME = @p1;", schemaName); err != nil {
		return nil, tracerr.Wrap(err)
	}
	return &dSchemas, nil
}
...
func (s *MssqlService) getTables(db *sqlx.DB, schemas *[]MsSqlSchema, dbName string) (*[]MsSqlTable, error) {
	...
	query, args, err := sqlx.In(`
		select * from INFORMATION_SCHEMA.TABLES where TABLE_CATALOG = @p1 and TABLE_SCHEMA in (@p2) order by TABLE_NAME`,
		dbName, dSchemaNames)
	if err != nil {
		return nil, tracerr.Wrap(err)
	}
